...
class Autoencoder(nn.Module):
	...
	def __init__(self, dimension):
		...
		self.encoder = nn.Sequential(nn.Linear(self.dim, self.dim//4), nn.ReLU(True), nn.Linear(self.dim//4, self.dim//16), nn.ReLU(True), nn.Linear(self.dim//16, self.dim//64))
		self.decoder = nn.Sequential(nn.Linear(self.dim//64, self.dim//16), nn.ReLU(True), nn.Linear(self.dim//16, self.dim//4), nn.ReLU(True), nn.Linear(self.dim//4, self.dim))
